...
if (!(Number.isInteger(maximumX) && Number.isInteger(maximumY))) {
  console.log('Grid Size should be an integer');
  return;
}
if (isNaN(currentX) || isNaN(currentX)) {
  console.log('Initial Position should be an integer');
  return;
}
for (i = 0; i < numOfSteps; i++) {
  if (traversalString[i] !== 'M' || traversalString[i] !== 'L' || traversalString[i] !== 'R') {
    console.log('The traversal String is not appropriate');
  }
